...
async def change_mode(mode: str):
    ...
    if mode == "silent":
        ...
        for fan in config.FANS:
            if fan["enabled"]:
                await ilo_ssh_command(f"fan p {fan['id']} max 51") # ~ 20% of 255
                ilo_state["manual_speeds"][fan["name"]] = 51 #config.SILENT_MODE_SPEED

        # Set the mode to silent
        ilo_state["mode"] = "silent"

    elif mode == "manual":
        # Set Max of fans in manual speeds
        for fan in config.FANS:
            if fan["enabled"]:
                await ilo_ssh_command(f"fan p {fan['id']} max {ilo_state['manual_speeds'][fan['name']]}")

        # Set the mode to manual
        ilo_state["mode"] = "manual"

    else:
        # Set all fans to max 100%, controlled by iLO PIDs
        for fan in config.FANS:
            if fan["enabled"]:
                await ilo_ssh_command(f"fan p {fan['id']} max 255")
...
async def set_manual_speed(fan_id: int, speed: int):
    ...
    # Define fan max speed
    await ilo_ssh_command(f"fan p {fan['id']} max {pwm_value}")
